...
void getOutputs(controller_t *controls, sensors_t *sensors, actuation_t *out)
{
  ...
  float fy = clamp(controls->fy, -1, 1); 
  float tauz = clamp(controls->tz, -1, 1);

  float L = 0.2425;
  float b = sqrt(2)/2;
  float f1 = -1.0/(4*b) * fx - 1.0/(4*b) * fy + 1.0/(4*L) * tauz;
  float f2 = 1.0/(4*b) * fx - 1.0/(4*b) * fy - 1.0/(4*L) * tauz;
  float f3 = 1.0/(4*b) * fx + 1.0/(4*b) * fy + 1.0/(4*L) * tauz;
  float f4 = -1.0/(4*b) * fx + 1.0/(4*b) * fy - 1.0/(4*L) * tauz;
